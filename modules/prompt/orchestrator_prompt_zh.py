...
## 调度规则（按优先级排序）
1. **打破二人循环（最高优先级）**：
   - 检查最近4条记录，如果都是同样的两个人（A-B-A-B），**必须**选择第三个人发言
   - 在打破循环时，**优先选择用户角色**，其次是其他未发言的Agent
   
2. **自然对话流**：
   - 如果没有形成二人循环，优先选择被提问的对象或话题相关者
   
3. **避免冷落用户**：
   - 如果用户超过3轮未发言，且当前话题可以被用户接话，优先选择用户
   
4. **平衡原则**：
   - 刚刚发言的角色被选中的可能性降低

## 输出要求
- 返回角色的名字（如"用户_qwer"、"小舞"等），**不要返回role_code**
- **只返回名字**，不要附加任何其它信息、解释或标点符号
"""
